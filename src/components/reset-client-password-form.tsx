...
export function ResetClientPasswordForm({ clientId, userId }: Props) {
  const initialPassword = useMemo(() => generateSuggestedPassword(), []);
  ...
  return (
    <form
      ...
      className="mt-2 grid gap-2 md:grid-cols-[1fr_auto_auto_auto] md:items-center"
      onSubmit={(event) => {
        const ok = window.confirm("Se restablecera la contraseña y se cerraran sesiones activas. Continuar?");
        if (!ok) {
