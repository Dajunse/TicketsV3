...
export default async function DashboardPage() {
  ...
  const [upcomingActivities, recentDone, openTickets, closedTickets, docs] = await Promise.all([
    prisma.activity.findMany({
      where: where ? { ...where, dueDate: { gte: new Date() } } : undefined,
      orderBy: { dueDate: "asc" },
      take: 5,
    }),
    prisma.activity.findMany({
    ...
    <section className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-sm text-zinc-600">
            Resumen ejecutivo de actividades, soporte y documentos.
          </p>
        </div>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <div className="rounded-xl border border-zinc-200 bg-white p-4">
          <p className="text-xs uppercase tracking-[0.15em] text-zinc-500">Actividades proximas</p>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <article className="space-y-3 rounded-xl border border-zinc-200 bg-white p-4">
          <h2 className="font-title text-lg text-zinc-900">Proximas actividades</h2>
          <div className="space-y-2">
            ...
            {upcomingActivities.length === 0 ? (
              <p className="text-sm text-zinc-500">No hay actividades proximas.</p>
            ) : null}
          </div>
        </article>
        ...
        <article className="space-y-3 rounded-xl border border-zinc-200 bg-white p-4">
          ...
          <div className="space-y-2">
            ...
            {recentDone.length === 0 ? (
              <p className="text-sm text-zinc-500">Aun no hay actividades completadas.</p>
            ) : null}
          </div>
        </article>
      </div>
